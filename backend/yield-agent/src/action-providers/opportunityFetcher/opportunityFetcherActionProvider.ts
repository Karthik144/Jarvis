 ...
 */
export class OpportunityFetcherActionProvider extends ActionProvider {
  constructor() {
    super("opportunities", []);
  }
...
`,
  ...
  async fetchOpportunities(): Promise<string> {
    ...
    try {
      const { data, error } = await supabase.from("base_uniswapv3_opps")
        .select(`
        data->poolAddress,
        data->totalAPR,
        data->token0Address,
...
export const opportunityFetcherActionProvider = () =>
  new OpportunityFetcherActionProvider();
