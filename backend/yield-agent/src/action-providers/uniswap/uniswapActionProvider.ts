...
import { getZapInRoute, buildZapInRoute } from "../zapInUtils";
...
`,
  ...
  async removeLiquidity(
    ...
    try {
      ...
      const canRemovePositition = await getPositionRemovalStatus(
        args.userId,
        args.tokenId
      );
      ...
      if (liquidity > BigInt(0)) {
        txs.push(
          await executeTransaction(
            wallet,
            encodeDecreaseLiquidity(tokenId, liquidity)
          )
        );
      }
      ...
      txs.push(
        await executeTransaction(
          wallet,
          encodeCollectTokens(tokenId, await wallet.getAddress())
        )
      );

      // Step 5: Burn the position
      const burnTx = await executeTransaction(
        wallet,
        encodeBurnPosition(tokenId)
      );
      ...
      return createSuccessResponse(
        tokenId,
        txs,
        burnReceipt.blockNumber.toString()
      );
    } catch (error) {
...
`,
  ...
  async createPosition(
    ...
    try {
      // To-Do: Remove token0 and token1 from args
      // 1. Pool Parameters
      const { token0, token1, fee, tickSpacing, currentTick } =
        await this.getPoolParameters(args.poolAddress);

      console.log(
        "Currently creating a uniswapv3 position with the following args",
        args
      );
      ...
      // 2. Tick Calculation
      // To-Do: Calculate tickLower and tickUpper based on user's risk tolerance
      // For now, we will use a fixed offset
      const tickOffset = 10;
      const tickLower = this.calculateTick(
        currentTick,
        tickSpacing,
        -tickOffset
      );
      const tickUpper = this.calculateTick(
        currentTick,
        tickSpacing,
        tickOffset
      );
      ...
      // 3. Get token info - change this
      const amountIn = parseEther(args.amountIn).toBigInt();
      const senderAddress = await wallet.getAddress();

      // 4. Get best zap-in route from KyberSwap API
      const zapRouteResp = await getZapInRoute({
        poolId: args.poolAddress,
      ...
      // 5. Approve router if needed (skip if native ETH)
      if (args.tokenIn !== "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE") {
        await approve(
          wallet,
          args.tokenIn,
          zapRouteResp.routerAddress,
          amountIn
        );
      }

      // 6. Build zap transaction
      const zapBuildResp = await buildZapInRoute({
        route: zapRouteResp.route,
      ...
      const tx = await wallet.sendTransaction({
        to: zapBuildResp.routerAddress,
        data: zapBuildResp.callData,
        value:
          args.tokenIn === "0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE"
            ? BigInt(zapBuildResp.value)
            : BigInt(0),
      ...
      const amount0Desired = (parseFloat(args.amountIn) / 2).toString();
      const amount1Desired = (parseFloat(args.amountIn) / 2).toString();
...
`,
  ...
  private async removePositionFromDatabases(
    userId: string,
    tokenId: string
  ): Promise<void> {
  ...
  // createPosition func's helper functions
